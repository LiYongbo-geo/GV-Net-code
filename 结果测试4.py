...
if __name__ == '__main__':
    nzyx = (32,64,64)
    density_modelfour = np.zeros(shape = [32, 64 ,64])
    density_modelfour[4:12, 15:25, 15:25] = 0.8
    #density_modelfour[16:20, 15:25, 15:25] = 0.8
    density_modelfour[6:14, 40:50, 40:50] = 0.8
    den_modelfour = torch.from_numpy(density_modelfour)
    den_modelfour = den_modelfour.unsqueeze(0)
    Forward = decoder.GravDecoder()
    field_modelfour = Forward.forward(data_input = den_modelfour)
    field_modelfour = torch.sum(field_modelfour,axis=1).squeeze(0)
    field_modelfour = field_modelfour.squeeze(0)

    field_modelfour = field_modelfour.numpy()

    field_modelfour = np.load("realdata.npy")
    field_modelfour = field_modelfour.flatten()
    
    #field_modelfour = giutils.contaminate(field_modelfour, 0.1, percent = True, seed = 62)
    field_modelfour = field_modelfour.reshape(1, 1, 64, 64)
    total_field = torch.from_numpy(field_modelfour)
    print("1")
    ...
    with torch.no_grad():
        ...
        #total_field = field_modelfour.unsqueeze(0)
        ...
        print("预测耗时：", end_time - start_time)
        #density = den_modelfour.unsqueeze(0)
        ...
        np.save("modefour_rec_real2.npy", tru_model)