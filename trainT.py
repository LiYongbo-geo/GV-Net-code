...
def load_args():
    ...
    parser.add_argument('-c', '--checkpoint', default='./models/checkpoint001.pt',
...
class DensityDataset(Dataset):
    ...
    def __next__(self):
        ...
        testfile = os.path.join(os.getcwd(), 'traindatasets_smallmodel', "models", modeltest)
        density = np.load(testfile).astype(np.float64)

        gravtest = 'field{}.npy'.format(str(self.current_idx))
        gravfile = os.path.join(os.getcwd(), 'traindatasets_smallmodel', "fields", gravtest)
...
def train(rank,world_size,args):  
    ...
    writer = SummaryWriter('runs/experiment000')
